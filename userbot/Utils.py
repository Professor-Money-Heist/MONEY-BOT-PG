...
from userbot import CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, Money, Userbot, bot
...
def load_addons(shortname):
    if shortname.startswith("__"):
        ...
        name = "userbot.plugins.Userbot-Addons.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Userbot-Addons." + shortname] = mod
...
def load_abuse(shortname):
    if shortname.startswith("__"):
        ...
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("Userbot-Abuse ~ " + shortname)
    else:
        import importlib
        import sys
        from pathlib import Path
