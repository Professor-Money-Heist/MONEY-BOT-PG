...
MONEY_logo = "MONEY_logo1"


@MONEY_HEIST_BOT.on(admin_cmd(pattern=r"listcmnd"))
@MONEY_HEIST_BOT.on(sudo_cmd(pattern=r"listcmnd", allow_sudo=True))
async def install(event):
    ...
    if len(OUTPUT) > 69:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            ...
            await edit_or_reply(
                MONEY_file,
                f"Output Too Large. This is the file for the list of plugins in MONEY-HEIST-BOT.\n\n**BY :-** {DEFAULTUSER}",
            )
            await event.delete()
