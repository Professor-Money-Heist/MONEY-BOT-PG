...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        MONEY = await edit_or_reply(ids, "Trying to gban this retard!")
    else:
        MONEY = await edit_or_reply(ids, "`Ok! Gban ho rha h intzarrr karo....`")
    ...
    try:
        ...
        return await MONEY.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        if user.id == 2105506225:
            return await MONEY.edit(f"`How dare u trying to Gban my master`")
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await MONEY.edit(f"Gbaning ho rha h 😂\n\nTotal Chats :- `{a}`")
            except:
                b += 1
    else:
        await MONEY.edit(f"`Either reply to a user or gib me user id/name`")
    try:
        if gmute(user.id) is False:
            return await MONEY.edit(
...
async def gunban(userbot):
    ...
    hum = await ids.client.get_me()
    if not sender.id == hum.id:
        MONEY = await edit_or_reply(ids, "`Trying to ungban this kid...`")
    else:
        MONEY = await edit_or_reply(ids, "`Ungban in progress...`")
    hum = await userbot.client.get_me()
    await MONEY.edit(f"`Trying to ungban this kiddo...`")
    my_mention = "[{}](tg://user?id={})".format(hum.first_name, hum.id)
    f"@{hum.username}" if hum.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    try:
        ...
        return await MONEY.edit("**Som3ting W3nt Wr0ng**")
    if user:
        if user.id == 2105506225:
            return await MONEY.edit(
        ...
        await MONEY.edit("**Reply to a user**")
    try:
        if ungmute(user.id) is False:
            return await MONEY.edit("**Error! User already ungbanned.**")
    except:
        pass
    return await MONEY.edit(
